...
\noindent A version of this material has been published as two chapters in the
book \emph{Parallel and Distributed Computing} edited by Charles
C. Weems Jr., Arnold Rosenberg, Sushil Prasad, Anshul Gupta, and Alan
Sussman.  As a result, the copyright and all other rights have been
transferred to Elsevier. \emph{However,} the author has been explicitly
given permission to provide this draft of the materials on this
website so that they can continue to be useful in this format for
...
programs are written.  Probably every program you have written in Java
...
it is empty.  In Java, you might naively write:
\begin{verbatim}
    while(true) {
       if(ovenIsEmpty()) {
          putCasseroleInOven();
...
may have multiple cores, but all the Java constructs you know are for
sequential programs, which do only one thing at once.  Before showing
any Java specifics, we need to explain the \emph{programming model}.
...
\begin{itemize}
\item There may be more threads than processors.  It is up to the Java
  ...
  the Java programmer's control: you create the threads and the system
  schedules them.
...
We finish this section with some Java specifics for exactly how to
create a new thread in Java.  The details vary in different languages
and in fact the parallelism portion of these notes mostly uses a
different Java library with slightly different specifics.  In addition
...
To create a new thread in Java requires that you define a new class
(step 1) and then perform two actions at run-time (steps 2--3):
\begin{enumerate}
\item Define a subclass of {\tt java.lang.Thread} and override the
  {\tt public} method {\tt run}, which takes no arguments and has
  return type {\tt void}.  The {\tt run} method will act like ``main''
  for threads created using this class.  It must take no arguments,
  but the example below shows how to work around this inconvenience.
\item Create an instance of the class you defined in step 1.  That is,
  if you defined class {\tt ExampleThread}, then use {\tt new} to create an {\tt ExampleThread}
  object.  Note this does \emph{not} yet create a running thread.  It
  just creates an object of class {\tt ExampleThread}, which is a subclass of {\tt
  Thread}.
\item Call the {\tt start} method of the object you created in step
  2.  This step does the ``magic'' creation of a new thread.  That new
  thread will execute the {\tt run} method of the object.  Notice that
  you do \emph{not} call {\tt run}; that would just be an ordinary method
  call.  You call {\tt start}, which makes a new thread that runs {\tt
    run}.  The call to {\tt start} ``returns immediately'' so the
  caller continues on, in parallel with the newly-created thread
  running {\tt run}.  The new thread terminates when its {\tt run}
  method completes.
\end{enumerate}
Here is a complete example of a useless Java program that starts with one
...
order, but it is up to the Java implementation to let all the
...
\end{verbatim}
This is really a question of how the {\tt System.out.println} method
...
must override {\tt java.lang.Thread}'s {\tt run} method and
therefore not take any arguments.  The standard idiom is to pass any
``arguments'' for the new thread to the \emph{constructor}, which then
stores them in fields so that {\tt run} can later access them.  In
this simple example, this trick is the only use of shared memory since
the helper threads (the ones doing the printing) do not need to
pass any other data to/from the main thread or each other.

It may not look like this is using shared memory, but it is: When the
main thread calls {\tt new ExampleThread(i)}, this is a normal call to a
constructor.  A new object gets created and the main thread runs the
constructor code, writing to the {\tt i} field of the new object.
Later, after the ``magic'' call to {\tt start}, the new running thread
is running the {\tt run} method with {\tt this} bound to the same
object that the main thread wrote to.  The helper thread then reads
{\tt i}, which just means {\tt this.i}, and gets the value previously
written by the main thread: shared-memory communication.  See
Figure~\ref{fig:aliasing_shared_memory}.

\begin{figure}
\begin{center}
\includegraphics[scale=.6]{images/aliasing_shared_memory.png}
\end{center}
\caption{How we can use shared memory, specifically fields in a subclass
  of {\tt Thread}, to pass data to a newly created thread.}
\label{fig:aliasing_shared_memory}
\end{figure}

As this example shows, shared-memory communication relies on
aliasing.  The {\tt t} on some iteration of the for-loop refers to the
same object as {\tt this} in one of the new threads' {\tt run}
method.  Aliasing is often difficult to reason about, but it is the
way we communicate in shared-memory multithreaded programs.
...
how using threads in Java to achieve the same idea requires a bit more
work (Section~\ref{sec:java-threads}).  We then argue that it is best
...
(Section~\ref{sec:divide-conquer}).  Because Java's threads are not
engineered for this style of programming, we switch to the Java
ForkJoin framework which is designed for our needs
(Section~\ref{sec:java-fjf}).  With all of this discussion in terms of
...
Unfortunately, Java and most other general-purpose languages do not
have a {\tt FORALL} loop.  (Java has various kinds of for-loops, but
...
creates 4 instances of {\tt SumThread}, which is a subclass of {\tt
  java.lang.Thread}, and calls {\tt start} on each of them.  The {\tt
  SumThread} constructor takes as arguments the data that the thread
needs to do its job, in this case, the array and the range for which
this thread is responsible.  (We use a convenient convention that
ranges \emph{include} the low bound and \emph{exclude} the high
bound.)  The {\tt SumThread} constructor stores this data in fields of
the object so that the new thread has access to them in the {\tt run}
method.  (Typically {\tt run} would pass the relevant data to helper
methods, but here the problem is simple enough not to bother.)

Notice each {\tt SumThread} object also has an {\tt ans} field.  This
is shared memory for communicating the answer back from the helper
thread to the main thread.  In particular, for the thread created on
the {\tt i}th iteration of the for-loop in {\tt sum}, the field {\tt
  this.ans} is an alias for {\tt ts[i].ans}.  So the main thread can
sum the 4 {\tt ans} fields from the threads it created to produce the
final answer.
...
{\tt ans} fields.  Remember that {\tt start} returns immediately ---
...
There is a method in {\tt java.lang.Thread}, and therefore in all its
subclasses such as {\tt SumThread}, that is just what we need.  If one
thread, in our case the main thread, calls the {\tt join} method of a
{\tt java.lang.Thread} object, in our case one of the helper threads,
then this call \emph{blocks} (i.e., does not return) unless/until the thread
corresponding to the object has terminated.  So we can add another
for-loop to {\tt sum} in-between the two loops already there to make
sure all the helper threads finish before we add together the results:
\begin{verbatim}
for(int i=0; i < 4; i++)
   ts[i].join();
\end{verbatim}
...
thread does not access a helper thread's {\tt ans} field until at least that
helper thread has terminated.

There is one last Java-specific detail we need when using the {\tt
  join} method defined in {\tt java.lang.Thread}.  It turns out this
method can throw a {\tt java.lang.InterruptedException} so a method
calling {\tt join} will not compile unless it catches this exception
or declares that it might be thrown.  For concurrent programming, it
may be bad style to ignore this exception, but for basic parallel
programming like we are doing, this exception is a nuisance and will
not occur.  So we will say no more about it.  Also the ForkJoin
framework we will use starting in Section~\ref{sec:java-fjf} has a
different {\tt join} method that does not throw exceptions.
...
the {\tt SumThread} class.  This example shows many of the key
...
performance.  Sections~\ref{sec:divide-conquer} and~\ref{sec:java-fjf}
...
Section~\ref{sec:java-fjf} are unavailable.  Fortunately, such
...
decide the number.  (There are Java library methods to ask for the
...
summing array elements.  It is fine to assume that the underlying Java
...
Sections~\ref{sec:divide-conquer} and~\ref{sec:java-fjf}.
\begin{enumerate}
...
\item Java's threads were not designed for small tasks like adding
  1000 numbers.  They will work and produce the correct answer, but
  the constant-factor overheads of creating a Java thread are far too
  large.  A Java program that creates 100,000 threads on a small
...
Java's threads.  Then Section~\ref{sec:java-fjf} switches to using a
...
With enough
processors, the total running time corresponds to the tree \emph{height}, not
the tree \emph{size}: this is the fundamental running-time benefit of
parallelism.  Later sections will discuss why the problem of summing
an array has such an efficient parallel algorithm; not every problem
enjoys exponential improvement from parallelism.
...
implementation with Java threads and then modify it with two important
...
To start, here is the algorithm directly translated into Java,
omitting some boilerplate like putting the main {\tt sum} method in a
class and handling {\tt java.lang.InterruptedException}.\footnote{For
  the exception, you cannot declare that {\tt run} throws this
  exception because it overrides a method in {\tt java.lang.Thread} that
  does not have this declaration.  Since this exception is not going
  to be raised, it is reasonable to insert a {\tt catch} statement and
  ignore this exception.  The Java ForkJoin Framework introduced in
  Section~\ref{sec:java-fjf} does not have this problem; its {\tt
    join} method does not throw checked exceptions.}
\begin{verbatim}
class SumThread extends java.lang.Thread {
  int lo; // fields for communicating inputs
  int hi; 
...
{\tt left.join} \emph{and} {\tt right.join}.  If for example, {\tt
  ...
  As a minor but important coding point, notice that the ``main'' {\tt
    sum} method calls the {\tt run} method directly.  As such, this is
  an ordinary method call like you have used since you started
  programming; the caller and callee are part of the same thread.  The
  fact that the object is a subclass of {\tt java.lang.Thread} is only
  relevant if you call the ``magic'' {\tt start} method, which calls
    {\tt run} in a new thread.
...
especially because the ForkJoin Framework in the next section performs
...
since we can just call the {\tt run} method directly, which recall is
just an ordinary method call unlike the ``magic'' {\tt start} method.
\begin{verbatim}
class SumThread extends java.lang.Thread {
  static int SEQUENTIAL_CUTOFF = 1000;
  int lo; // fields for communicating inputs
...
\end{verbatim}
Notice how the code above creates two {\tt SumThread} objects, but only
creates one helper thread with {\tt left.start()}.  It then does the
right half of the work itself by calling {\tt right.run()}.  There is
...
the work are done in parallel.  Creating a {\tt SumThread} object for
...
implementation of Java's threads is not engineered for threads that do
...
uncommon for a Java implementation to pre-allocate the maximum amount
of space it allows for the call-stack, which might be 2MB or more.  So
...
programming.  We will return to Java's threads when we learn
concurrency because the synchronization operations we will use work
with Java's threads.

\subsection{The Java ForkJoin Framework}
\label{sec:java-fjf}

Java 7 (and higher) includes classes in the {\tt java.util.concurrent} package
...
threads (which the library calls ForkJoinTasks) that are small enough
...
languages include Intel's Thread Building Blocks, Microsoft's Task
Parallel Library for C\#, and others.  The core ideas and
...
specifics.  Compared to Java threads, the core ideas are all the same,
...
For introductory notes on using the library 
and avoiding some difficult-to-diagnose pitfalls, see
\url{http://www.cs.washington.edu/homes/djg/teachingMaterials/grossmanSPAC_forkJoinFramework.html}.
The examples in these notes below assume Java 8 or higher; the url includes a
small amount of extra code you need to modify these examples for Java
7 or Java 6.

We first show a full program (minus a {\tt main} method) that is as
much as possible like the version we wrote using Java threads.  We
...
second version that uses Java's generic types and a different library
...
While there are many differences compared to using Java's threads, the
overall structure of the algorithm should look similar.  Furthermore,
most of the changes are just different names for classes and methods:
\begin{itemize}
\item Subclass {\tt java.util.concurrent.RecursiveAction} instead
  of {\tt java.lang.Thread}.
\item The method that ``magically'' creates parallelism is called {\tt
    fork} instead of {\tt start}.
\item The method that starts executing when a new thread begins is
  called {\tt compute} instead of {\tt run}.  Recall these methods can
  also be called normally.
\item (The method {\tt join} is still called {\tt join}.)
\end{itemize}

The small additions involve starting the computation with a call to
{\tt ForkJoinPool.commonPool().invoke}
These are just some details because
the library is not built into the Java \emph{language}, so we have to
do a little extra to initialize the library and start using it.  Here
is all you really need to know:
\begin{itemize}
\item The entire program should have exactly one {\tt ForkJoinPool},
  so it makes sense to store it in a static field and use it for all
  the parallel algorithms in your program.  The library's {\tt
  commonPool} static method returns such an object for you.
\item Inside a subclass of {\tt RecursiveAction}, you use {\tt fork},
  {\tt compute}, and {\tt join} just like we previously used {\tt
    start}, {\tt run}, and {\tt join}.  But outside such classes, you
  cannot use these methods or the library may not work correctly.  To
  ``get the parallelism started,'' you instead use the {\tt invoke} method
  of the {\tt ForkJoinPool}.  You pass to {\tt invoke} a subclass of {\tt
    RecursiveAction} and that object's {\tt compute} method will be
  called.  Basically, use {\tt invoke} once to start the algorithm and
  then {\tt fork} or {\tt compute} for the recursive calls.
  Conversely, do \emph{not} use {\tt invoke} from ``inside'' the
  library, i.e., in {\tt compute} or any helper methods it uses.
\end{itemize}

We will present one final version of our array-summing program to
demonstrate one more class of the ForkJoin Framework that you should
use as a matter of style.  The {\tt RecursiveAction} class is best
only when the subcomputations do not produce a result, whereas in our
example they do: the sum of the range.  It is quite common not to
produce a result, for example a parallel program that increments every
element of an array.  So far, the way we have ``returned'' results is
via a field, which we called {\tt ans}.  

Instead, we can subclass {\tt RecursiveTask} instead of {\tt
  RecursiveAction}.  However, {\tt RecursiveTask} is a generic class
with one type parameter: the type of value that {\tt compute} should
return.  Here is the full version of the code using this more
  convenient and less error-prone class, followed by an explanation:
...
Here are the differences from the version that subclasses {\tt
  RecursiveAction}:
\begin{itemize}
\item {\tt compute} returns an object of whatever (object) type we
  want.  Because {\tt int} is not an object type, we write {\tt
    Integer} instead.  Java implicitly converts an {\tt int} to/from
  an {\tt Integer} in all the places it is needed in this program.
\item The {\tt join} method also returns the value returned by {\tt
    compute} in the thread that is being joined to.  Therefore, we do
    not need an {\tt ans} field: we write {\tt int leftAns =
    left.join()} to get the value that the thread bound to {\tt left}
    returned from its {\tt compute} method.
\item The {\tt invoke} method works similarly when passed a subclass
  of a {\tt RecursiveTask} class: It returns the value of the {\tt
  compute} method that is called.
\item The class declaration of {\tt SumArray} indicates that it
  extends {\tt RecursiveTask<Integer>}.  More generally, always
  put the same type between the angle brackets as the return type of {\tt
    compute}.
\item Note that because Java expressions are always evaluated
  left-to-right, we could replace the last 3 lines of the else-branch
  in {\tt compute} with just {\tt return right.compute() +
  left.join();}.  But this is rather subtle --- {\tt left.join() +
  right.compute()} would destroy the parallelism --- so we do not
  advise writing this shorter but less clear code.
\end{itemize}

If you are familiar with Java's generic types, this use of them should
not be particularly perplexing.  The library is also using static
overloading for the {\tt invoke} method.  But as \emph{users} of the
library, it suffices just to follow the pattern in the example above.

Why is there {\tt RecursiveAction} and {\tt RecursiveTask}?  The
former is better style when there really is nothing to return;
otherwise {\tt RecursiveTask} is better.  Both are actually
implemented in terms of the same superclass inside the library.  You
can use that class directly, but it is less convenient.

Given the convenience of not needing a field for returning values, why
not also provide the convenience of not needing fields to pass
arguments ({\tt arr}, {\tt lo}, and {\tt hi})?  That \emph{would} be
nice, but there just isn't a particularly pleasant way to write such a
library in Java.  The ForkJoin framework is just a library; it cannot
make any changes/extensions to the Java language.  It uses some
advanced Java features to be as convenient as it can be.
...
programming, and the fork-join framework has several classes providing
...
\end{verbatim}
Coding up this algorithm in the ForkJoin framework is straightforward:
...
there are no other results to combine, subclassing {\tt
  RecursiveAction} is appropriate.  Using a sequential cut-off and
...
ForkJoin framework or Hadoop to do the actual scheduling of the
parallelism.
